...
> ⚠️ **Wave 4**: 以下 Level 1/2 信任分级和 step review 为 Wave 4 范围。
> Wave 1 统一使用简单费用确认 dialog（无信任引擎、无 step review）。
...
> ⚠️ **Wave 4**: 信任级别设置为 Wave 4 范围。Wave 1 使用简单费用确认。
...
> ⚠️ **Wave 2**: 以下 Review 模式（质量自评绿/黄点 + Review & Tweak）不在 Wave 1。
> Wave 1 完成后直接回到 Storyboard Grid 查看生成结果。
...
> ⚠️ **Wave 2**: Layer 2 整体为 Wave 2 范围。Wave 1 用户通过 Storyboard Grid 手动处理问题 shot。
...
> ⚠️ **Wave 3+**: Layer 3 的 Director's Notes、Prompt Preview、模型智能排序为 Wave 3+ 范围。
> Wave 1 用户通过现有的 shot-detail-panel 手动编辑参数（与当前 UI 相同）。
...
> ⚠️ **v2 愿景旅程**: 以下旅程描述了全部 Wave 实现后的理想状态。
> Wave 1 的实际旅程: 写剧本 → Create My Video → 确认费用 → 等待 → 在 Grid 中查看结果。
> 质量评估、推荐修复、偏好学习等为 Wave 2-4 功能。
...
5. 看到结果                              ← Wave 1 到此为止
6. 如果满意 → Export（done!）
7. 如果 1-2 个 shot 不好 → 点击 → 选推荐修复 → 等待  ← Wave 2+
8. Export

总交互: Wave 1 ~15 次 / 全部 Wave ~12-20 次（vs 当前 60 次）
```
