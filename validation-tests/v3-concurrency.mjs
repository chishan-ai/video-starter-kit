...
const CHARACTER_URL =
  "https://v3b.fal.media/files/b/0a93b96d/EiwtsB4RINbQiA_fLXUUl.jpg";
...
async function submitJob(model, prompt, imageUrl, label) {
  ...
  try {
    const input = imageUrl ? { prompt, image_url: imageUrl } : { prompt };
...
async function main() {
  ...
  const t2vJobs = [
    submitJob(
      ...
      "anime girl with pink twin tails running through rain, dramatic lighting, anime style",
      null,
      "T2V-1",
    ),
    submitJob(
      "fal-ai/vidu/q3/text-to-video",
      "anime girl with pink twin tails sitting on rooftop at night, stars in sky, anime style",
      null,
      "T2V-2",
    ),
  ];
  ...
  console.log(
    `Total: ${totalOk} success, ${totalFail} failed, out of ${allResults.length}`,
  );
  console.log(
    `\nVidu Q3 (i2v, 5 parallel): ${viduOk}/5 — wall time ${t1Elapsed}s`,
  );
  console.log(
    `Kling 3.0 (i2v, 3 parallel): ${klingOk}/3 — wall time ${t2Elapsed}s`,
  );
  console.log(
    `Vidu Q3 (t2v, 2 parallel): ${t2vResults.filter((r) => r.status === "ok").length}/2 — wall time ${t3Elapsed}s`,
  );
  ...
  fs.writeFileSync(
    "./validation-tests/v3-results.json",
    JSON.stringify(
      {
